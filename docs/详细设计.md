...
**候选来源**（`collectEntries`，三路合并按路径去重，全部渲染层既有数据）：已打开标签（标记 `•`，回车跳转既有标签）→ 当前文件夹树（递归展平，仅文件节点）→ 最近列表（读盘新开，`openPath`）。
...
**已知伪影**：空单元格（新插入的行/列）经 milkdown gfm 序列化会输出 `<br />` 占位（上游库对「GFM 单元格不可为空」的补偿行为，其自带的加行命令同样如此）；同一行内出现 raw HTML 时相邻文本可能被 remark 转义为 code 包裹。填入真实内容后即恢复正常。
